...
class ClientSecureChannelLayer extends events_1.EventEmitter {
    ...
    closeWithError(err, statusCode, callback) {
        if (this._transport) {
    ...
    _on_message_received(response, msgType, requestId) {
        (0, node_opcua_assert_1.assert)(msgType !== "ERR");
        /* istanbul ignore next */
        if (response.responseHeader.requestHandle !== requestId) {
            errorLog(chalk.red.bgWhite.bold("xxxxx  <<<<<< _on_message_received  ERROR"), "requestId=", requestId, "response.responseHeader.requestHandle=", response.responseHeader.requestHandle, response.schema.name.padStart(30));
        }
        /* istanbul ignore next */
    ...
    _renew_security_token() {
        ...
        this._open_secure_channel_request(isInitial, (err) => {
            ...
            if (!err) {
                ...
                this.closeWithError(new Error("Restarting because Request has timed out during OpenSecureChannel"), node_opcua_transport_2.StatusCodes2.BadRequestTimeout, () => {
                    /* */
                });
            }
        });
