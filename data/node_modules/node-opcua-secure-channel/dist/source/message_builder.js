...
class MessageBuilder extends node_opcua_transport_1.MessageBuilderBase {
    ...
    _reportErrMessage(message) {
        try {
            const binaryStream = new node_opcua_binary_stream_1.BinaryStream(message);
            ...
            if (message.length === msgLength) {
                ...
                const additionalInfo = (0, node_opcua_basic_types_1.decodeString)(binaryStream);
                // invalid message type
                const m1 = `ERR: ${errorCode} ${additionalInfo}`;
                warningLog(m1);
                return this._report_error((0, node_opcua_status_code_1.coerceStatusCode)(errorCode), m1);
            }
            else {
                return this._report_error(node_opcua_status_code_1.StatusCodes.BadTcpInternalError, message.toString("hex"));
            }
        }
        catch (err) {
            console.log((0, node_opcua_debug_1.hexDump)(message));
            return this._report_error(node_opcua_status_code_1.StatusCodes.BadTcpInternalError, message.toString("hex"));
        }
    }
    _decodeMessageBody(fullMessageBody) {
        ...
        if (msgType === "HEL" || msgType === "ACK") {
            // invalid message type
            return this._report_error(node_opcua_transport_1.StatusCodes2.BadTcpMessageTypeInvalid, "Invalid message type ( HEL/ACK )");
        }
        ...
        try {
            ...
            return this._report_error(node_opcua_transport_1.StatusCodes2.BadTcpInternalError, err instanceof Error ? err.message : " err");
        }
    ...
    _decrypt_OPN(binaryStream) {
        ...
        if (asymmetricAlgorithmSecurityHeader.receiverCertificateThumbprint) {
            ...
            (0, node_opcua_assert_1.assert)(this.privateKey !== invalidPrivateKey, "expecting a valid private key");
