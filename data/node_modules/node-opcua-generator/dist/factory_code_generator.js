...
const write_func_1 = require("./utils/write_func");
...
function write_enumeration_setter(write, schema, field, member) {
    const capMember = (0, node_opcua_utils_1.capitalizeFirstLetter)(member);
...
function write_enumeration_fast_init(write, schema, field, member, i) {
    write(`             this.${member} =  0 as  ${field.fieldType};`);
}
function write_enumeration(write, schema, field, member, i) {
    (0, node_opcua_assert_1.assert)(!field.isArray); // would not work in this case
    const capMember = (0, node_opcua_utils_1.capitalizeFirstLetter)(member);
    write(`        this.${field.name} = this.set${capMember}(initialize_field(schema.fields[${i}], options?.${field.name}));`);
}
function write_complex_fast_init(write, schema, field, member) {
    if (field.isArray) {
...
function write_basic(write, schema, field, member, i) {
    (0, node_opcua_assert_1.assert)(field.category === node_opcua_factory_1.FieldCategory.basic);
    if (field.isArray) {
        // write(`this.${member} = [];`);
        // write(`if (options.${member}) {`);
        // write(`    assert(Array.isArray(options.${member}));`);
...
function write_basic_fast_init(write, schema, field, member, i) {
    if (field.isArray) {
        // write(`this.${member} = [];`);
        // write(`if (options.${member}) {`);
        // write(`    assert(Array.isArray(options.${member}));`);
...
function write_fast_init_member(write, schema, field, i) {
...
function write_decode(write, schema) {
    ...
    function write_field(field, member, i) {
        if (field.category === node_opcua_factory_1.FieldCategory.enumeration || field.category === node_opcua_factory_1.FieldCategory.basic) {
            ...
            (0, node_opcua_assert_1.assert)(field.category === node_opcua_factory_1.FieldCategory.complex);
            if (field.isArray) {
...
function getDataTypeNodeId(schema) {
    const className = schema.name;
    const encodingBinaryId = node_opcua_constants_1.DataTypeIds[className];
    return (0, node_opcua_nodeid_1.coerceNodeId)(encodingBinaryId);
}
function getEncodingBinaryId(schema) {
    const className = schema.name;
    const encodingBinaryId = node_opcua_constants_1.ObjectIds[className + "_Encoding_DefaultBinary"];
    return (0, node_opcua_nodeid_1.coerceNodeId)(encodingBinaryId);
}
function getEncodingXmlId(schema) {
    const className = schema.name;
    const encodingXmlId = node_opcua_constants_1.ObjectIds[className + "_Encoding_DefaultXml"];
    return (0, node_opcua_nodeid_1.coerceNodeId)(encodingXmlId);
}
...
function produce_TScript_code(schema, localSchemaFile, generatedTypescriptFilename) {
    ...
    const baseClass = schema.baseType;
    (0, node_opcua_factory_1.check_schema_correctness)(schema);
    const f = new index_1.LineFile1();
    ...
    for (const field of complexTypes) {
        ...
        const localFilename = (0, index_1.normalize_require_file)(folderForSourceFile, filename);
    ...
    if (baseClass !== "BaseUAObject") {
        ...
        if (fs.existsSync(filename)) {
            (0, node_opcua_assert_1.assert)(!localFilename.match(/\\/));
